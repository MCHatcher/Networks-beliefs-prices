...
K = 160;
%K = 160; % No. of chartists
run bipartite_K; g_init = zeros(1,n); %g_init(1:K) = 2.4;
g_init(1:K) = 2.15;  %Initial distribution of g
...
figure(1)
subplot(2,2,3), plot(gama_stack, consensus, '','Color',[grey grey grey],'LineWidth',1),  hold on,
title('Consensus type'), xlabel('\gamma'), axis([0,max(gama_stack),0,inf]), hold on,
yline(1+r)
